					...
					<view v-for="(item,idx) in list" :key="idx">
						<view class="meOverBg" @tap="transportmess(item)">
							<view class="meVisitor">
								<view class="meVisitorTitle" style="width: 50%;" >运货商：{{item.mname}}</view>
								<view class="line" style="width: 50%;padding:10rpx;border-radius:15rpx;color:#7f8388 ;text-align: right;">点击查看物流信息</view>
							</view>						
							<view class="meVisitor">							
	...
	export default{
		...
		methods: {
					...
					getdata(){
						...
						if(value){				
						  	uni.request({
						  	    url: config.httpUrl +'/api/v1/commodity/orderuserlist',
						  		method:"GET",
								data:{
									'type':'dealing'
								},
